...
class CustomGraphRAG:
    ...
    async def OPENAI_achat(self, messages, model="gpt-4o", temperature=0, config={}):
    ...
    async def achat(self, messages, model="microsoft/phi-4-reasoning-plus:free", temperature=0, config={}):
        """
        Process chat messages using the OpenRouter API with the specified model.

        Args:
            messages (list): List of message dictionaries with 'role' and 'content' keys
            model (str): Model identifier, defaults to microsoft/phi-4-reasoning-plus:free
            temperature (float): Sampling temperature
            config (dict): Additional configuration parameters (currently not directly used for OpenRouter in this function)
    ...
    def __enter__(self) -> "CustomGraphRAG":
        """
